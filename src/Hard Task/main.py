...
main.py
-------
Runs the Hard Task VAE multi-modal music clustering pipeline locally.
Uses music_dataset/ at the repo root instead of downloading from GTZAN / Kaggle / yt-dlp.

Expected dataset layout
-----------------------
    music_dataset/
    ├── gtzan/            # one sub-folder per genre
    │   ├── blues/
    │   │   ├── track1.wav
    │   │   └── ...
    │   ├── jazz/
    │   └── ...
    ├── banglagiti/       # 3 songs per genre is fine
    │   ├── Baul/
    │   ├── Folk/
    │   └── ...
    └── bmgcd/
...
GTZAN_DIR         = os.path.join(MUSIC_DATASET_DIR, 'gtzan')
BANGLAGITI_DIR    = os.path.join(MUSIC_DATASET_DIR, 'banglagiti')
BMGCD_DIR         = os.path.join(MUSIC_DATASET_DIR, 'bmgcd')
