...
clustering.py
-------------
Elbow method, K-Means clustering on VAE latent space, PCA + K-Means baseline,
t-SNE visualisation, UMAP visualisation, and cluster composition chart.
"""
...
def plot_elbow(Z, k_range=range(2, 16), out_dir='/content'):
    """
    Compute and plot inertia for K in k_range.
    Returns the inertia list (plot is decorative; K is set programmatically).
    """
...
def cluster_vae(Z, K):
    """
    Run K-Means on the VAE latent space.

    Parameters
    ----------
    Z : np.ndarray — latent representations (N, latent_dim)
    K : int — number of clusters (set to number of unique genres)

    Returns
    -------
    km_vae : fitted KMeans object
    labels_vae : np.ndarray of cluster labels
    """
...
def cluster_pca_baseline(X_scaled, K, latent_dim=16):
    """
    Run PCA (latent_dim components) + K-Means as a baseline.

    Parameters
    ----------
    X_scaled : np.ndarray — normalised feature matrix (N, D)
    K : int        — number of clusters
    latent_dim : int        — number of PCA components (matches VAE latent dim)

    Returns
    -------
    pca : fitted PCA object
    Z_pca : np.ndarray — PCA-reduced matrix (N, latent_dim)
    km_pca : fitted KMeans object
    labels_pca : np.ndarray of cluster labels
    """
...
def plot_tsne(Z, y_genre, y_lang, le_genre, le_lang, out_dir='/content'):
    """
    Fit t-SNE on the VAE latent space and plot by genre and language.
    Adaptive perplexity: min(30, N // 4).
    """
    os.makedirs(out_dir, exist_ok=True)
...
def plot_umap(Z, y_genre, y_lang, le_genre, le_lang, out_dir='/content'):
    """
    Fit UMAP on the VAE latent space and plot by genre and language.
    Adaptive n_neighbors: min(15, N - 1).
    """
    os.makedirs(out_dir, exist_ok=True)
...
# ════════════════════════════════════════════════════════════════════════════
# Cluster Composition — Language Mix per Cluster
# ════════════════════════════════════════════════════════════════════════════

def plot_cluster_composition(df, out_dir='/content'):
    """
    Bar chart showing Bangla vs English track counts per cluster,
    for both VAE and PCA clustering results.
    Requires df to have 'cluster_vae' and 'cluster_pca' columns already set.
    """
    os.makedirs(out_dir, exist_ok=True)
